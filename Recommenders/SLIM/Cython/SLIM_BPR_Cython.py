...
def get_RAM_status():
    ...
    f1 = 0.000000953674316   # 1/(1024*1024) Converts bytes to MB
    ...
    sy = subprocess.Popen(['sysctl','vm.page_pageable_internal_count'], stdout=subprocess.PIPE).communicate()[0].decode()
    p1 = sy.find(':')
    page_pageable_internal_count = float(sy[p1+1:50]) * 16384
    sy = subprocess.Popen(['sysctl','vm.swapusage'], stdout=subprocess.PIPE).communicate()[0].decode()
    p1 = sy.find('used')
    p2 = sy.find('M',p1)
    swapUsed = float(sy[p1+7:p2])   # MBytes

    # Pressure - just get the pressure value
    sy = subprocess.Popen(['memory_pressure'], stdout=subprocess.PIPE).communicate()[0].decode()
    p1 = sy.find('tage:')
    p2 = sy.find('%')
    ...
    try:
        tot_m = float((vmStats["Pages free"] + vmStats["Pages wired down"] + vmStats["Pages active"] + vmStats["Pages inactive"] + vmStats["Pages speculative"] + vmStats["Pages throttled"] + vmStats["Pages occupied by compressor"]) * f1 )
        
        used_m = float((appMemory + vmStats["Pages wired down"] + vmStats["Pages occupied by compressor"] + vmStats["File-backed pages"] + vmStats["Pages purgeable"] + vmStats["Pages free"]) * f1 * mp / 100)
        available_m = float(tot_m - used_m)
